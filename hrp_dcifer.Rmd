...
**Dcifer run**
Can skip this section if dcifer and moire were run already. Just import the "dcifer_results.rds"

```{r dcifer run, echo=FALSE, warning = FALSE, message= FALSE}
moire_input <- read.csv("HRP_moire_input.csv") 
...
# Input moire_results if already stored
moire_results <- readr::read_rds("/Users/williamlouie/Dropbox/My Mac (Williams-MacBook-Pro.local)/Downloads/Ethiopia/hrp_moire/moire_results.rds")
...
moire_input <- read.csv("/Users/williamlouie/Dropbox/My Mac (Williams-MacBook-Pro.local)/Downloads/Ethiopia/hrp_moire/HRP_moire_input.csv") 
dat <- moire::load_long_form_data(moire_input)
...
dres <- ibdDat(dsmp, coi, afreq, pval = TRUE, confint = TRUE, rnull = 0, alpha = 0.05, nr = 1e3)   

## Output dcifer results
readr::write_rds(dres, "dcifer_results.rds")

```

**Dicfer heatmap combine p-value & R**

```{r dcifer heatmap, exercise=TRUE}
...
```{r dcifer_var heatmap, exercise=TRUE}
...
```{r cluster network plot, exercise=TRUE}
...
```{r cluster group featured plot, exercise=TRUE}
...
setTimeLabels <- function(dcifer_list, filename){
  thisData <- read.csv(filename)
  time_labels <- as.vector(thisData[,4])
...
plotClusterGroup <- function(dcifer_list, level=0.5, alpha=0.05, vSize=2, thick = 2, vFontSize=.5, 
                             labelOffset=1, legend_x=2, legend_y=2, group_name = 'Group', showLabel = F){
  edges <- NULL
  wgts <- NULL
  vcolours <- NULL
  isolates <- dcifer_list$id
...
filename <- 'C:/Users/User1/SynologyDrive/UCSF work/Consulting/Rcodes/03_Dcifer_data/epi_info.csv' # Change to your filepath
...
plotClusterGroup(dcifer_list, level=0.9, alpha=0.05, vSize=3, thick = 2.5, labelOffset=1,legend_x=2, legend_y=2, group_name='District', showLabel = T) # Change the relatedness "level" based on your desire
